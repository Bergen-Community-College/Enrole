/*
 * @Description: Bergen Community College footer wrapper.
 * @Usage: <script type="text/javascript" src="style.js" id="bergen-wrapper-script" hide-header="true"></script>
 * @Attributes:
...
(function () {
	...
	if (document.querySelector('[id$="_footer"]')) return;

	let tag = document.getElementById('bergen-wrapper-script') || document.createElement('div'),
		hide_header = tag.getAttribute("hide-header") || false,
	...
	function add_wrapper() {
		if (!hide_header) {
			if (header_placement !== null && header_placement !== '') {
				...
				var loc = header_placement_segments[header_placement_segments.length-1];
				if (loc == header_placement) {
					applyHeader(loc);
				} else {
					switch (header_placement_segments[0]) {
						case 'before':  applyHeader(loc, 'before'); break;
						case 'prepend': applyHeader(loc, 'prepend'); break;
						case 'append':  applyHeader(loc, 'append'); break;
						default:        applyHeader(loc, 'after');
					}
				}
			} else {
		...
		if (footer_placement !== null && footer_placement !== '') {
			...
			var loc = footer_placement_segments[footer_placement_segments.length-1];
			if (loc == footer_placement) {
				applyFooter(loc);
			} else {
				switch (footer_placement_segments[0]) {
					case 'before':  applyFooter(loc, 'before'); break;
					case 'prepend': applyFooter(loc, 'prepend'); break;
					case 'append':  applyFooter(loc, 'append'); break;
					default:        applyFooter(loc, 'after');
				}
			}
		} else {
	...
	function applyHeader(id, position) {
		...
		if (el instanceof Object) {
			switch (position) {
				case 'before':  el.parentNode.insertBefore(header_host, el); break;
				case 'prepend': el.insertBefore(header_host, el.firstChild); break;
				case 'append':  el.appendChild(header_host); break;
				default:        el.parentNode.insertBefore(header_host, el);
			}
		} else {
